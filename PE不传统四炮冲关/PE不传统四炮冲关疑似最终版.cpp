/*
    写在最最前面，刚开始使用键控脚本的玩家请务必注意这里的注释，如果不看这里的注释导致脚本无法正常运行的情况，本作者有权不解答问题
    该脚本对应的阵型为PE不传统四炮，注意一定要按照我给的阵图摆放植物，不要少摆错摆，不要随意改阵，改动后破阵的情况请务必不要找我
    该脚本目前可以冲关数千f，还存在一些小问题，但我还不想改，以后想改了再说吧
    使用的avz版本：2022_06_30，不保证兼容其他版本（已知无法在2022_10_01版本中运行）
    使用的插件及其版本如下
    shallowdream.h          2022_10_20
    PlantFixerPlus.h        2022_08_16
    avz_more_autoplay.h     2022_08_22
    TickPlanter.h           2022_06_04

    插件使用方法：在AVZlib插件库（网址在下方）中找到对应版本的插件（.h文件）；将找到的插件放在AVZ安装目录下面的inc文件夹中；然后就可以运行脚本了
    AVZlib插件库网址：https://github.com/qrmd0/AvZLib
*/
#include <avz.h>
...
void ManageFixer2(PlantFixerPlus &fixer)
{
    ...
    // 启用自动修补的时间段
    const std::vector<TimeRange> enable_time_ranges = {{1, -200, 2500}, {2, -200, 2500}, {3, -200, 2500}, {4, -200, 2500}, {5, -200, 2500}, {6, -200, 2500}, {7, -200, 2500}, {8, -200, 2500}, {9, -200, 0}, {10, 401, 2500}, {11, -200, 2500}, {12, -200, 2500}, {13, -200, 2500}, {14, -200, 2500}, {15, -200, 2500}, {16, -200, 2500}, {17, -200, 2500}, {18, -200, 2500}, {19, -200, 600}, {20, 401, 2500}};
...
void SmartCard1() //依据血量放垫材
{
    for (int row : {1, 6})
    {
        if (IsZBExist(GIGA_GARGANTUAR, row, 500, 100) || IsZBExist(FOOTBALL_ZOMBIE, row, 500, 0) || IsZBExist(GARGANTUAR, row, 500, 100))
        {
        ...
        InsertOperation([=]()
            ...
            else if (c == 1)
            {
                ...
                else
                {
                    ...
                    InsertOperation([=]()
                    {
                        if ((GetPlantIndex(3, 6, 14) > 0 || GetPlantIndex(3, 6, 63) > 0 || GetPlantIndex(4, 6, 14) > 0 || GetPlantIndex(4, 6, 63) > 0) && (!GetZombieTypeList()[BUNGEE_ZOMBIE] || (GetZombieTypeList()[GIGA_GARGANTUAR] && !GetZombieTypeList()[GARGANTUAR] && wave != 19) || GetZombieTypeList()[GARGANTUAR] || (!GetZombieTypeList()[GIGA_GARGANTUAR] && !GetZombieTypeList()[GARGANTUAR] && wave != 9 && wave != 19)) && IsUsable(COFFEE_BEAN))
                        {
                            SetNowTime(); 
                            ice_filler.coffee();
                        }
...
void Script()
{
    ...
    //红眼或者白眼关关收尾一对炮杀伤巨人
    if (GetZombieTypeList()[GIGA_GARGANTUAR] || GetZombieTypeList()[GARGANTUAR])
    {
        for (int wave : {9, 19, 20})
        {
            ...
            InsertOperation([=]()
                            {
                if (IsHYExist(1, 0) || IsHYExist(6, 0) || IsBYExist(1, 0) || IsBYExist(6, 0))
                {
                    SetNowTime();
