...
PlantFixerPlus TWIN_SUNFLOWER_fixer;
...
void Script()
{
    ...
    //智能修补位于(4, 9)的双子向日葵，血量阈值为总血量的1%，不使用模仿种子
    TWIN_SUNFLOWER_fixer.start(TWIN_SUNFLOWER, {{4, 9}}, 0.01, false);

    //智能修补位于(4, 9)的双子向日葵，血量阈值为总血量的1%，不使用模仿种子
    LILY_PAD_fixer.start(LILY_PAD, {{4, 9}}, 0.01, false);

    plant_fixer.start(PUMPKIN, {{4, 9}}, 1000);
